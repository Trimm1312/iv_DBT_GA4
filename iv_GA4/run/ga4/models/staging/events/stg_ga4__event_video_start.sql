  ...
  create or replace view `set-ga-reporting`.`iv_GA4`.`stg_ga4__event_video_start`
  OPTIONS()
  as -- Defined as when the video starts playing. For embedded YouTube videos that have JS API support enabled. Collected by default via enhanced measurement.
